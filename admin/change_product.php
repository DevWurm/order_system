...
//local function declarations
	...
	function get_product_data() { //get user input from POST and return false if date is not set
		if (!isset($_POST['product_id'])) {
			...
			$product_data['product_id'] = $_POST['product_id'];
		}
		...
		if (isset($_POST['product_deprecated'])) {
			$product_data['product_deprecated'] = 1;
		}
		else {
			$product_data['product_deprecated'] = 0;
		}
		return $product_data;
	}
	...
	function validate_unique_product_name($product_data) {
		...
		if ($db_data != FALSE) {  //check if an error occured
			if (mysqli_num_rows($db_data)>0) { //check if the product name already exists
				...
				if ($data[0]['p_id'] == $product_data['product_id']) { //if the row with the same product name ifs the same as the updated (--> product_name not changed) return true
					return TRUE; // --> unique
				}
				else { //otherwise the product name would be a duplicate
	...
	function update_product($product_data) { //update product in database
		...
		if (!mysqli_query($db_link, "UPDATE products SET product_name='".$product_data['product_name']."', quantity='".$product_data['product_quantity']."', max_quantity='".$product_data['product_max_quantity']."', price='".$product_data['product_price']."', deprecated='".$product_data['product_deprecated']."' WHERE p_id='".$product_data['product_id']."'")) {
			return FALSE;
		}
		else {
			return TRUE;
...
//execute update
	...
	if ($product_data != FALSE) {
		if (validate_product_data($product_data)) {
			if (update_product($product_data)) {
							?>
			<!DOCTYPE html>
				<html lang="en">
					<head>
