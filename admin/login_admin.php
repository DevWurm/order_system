...
//local function declarations
	function login_admin() {
		$admin_data = get_login_data();
		
		if ($admin_data != FALSE) { //check if input data is correctly transfered
			if (validate_login_data($admin_data)) { //check if input data is valid
				if (validate_login($admin_data, 'admin')) { //check if login credentials are correct
			...
			die("FEHLER: Fehler bei der Übertragung der Daten");
		}
	}
?>
<?php
//execute login
	...
	if (login_admin()) { //check if loggin credential are correct
