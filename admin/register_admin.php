...
//local function declaration
 	...
 	function get_admin_data() { //get user input from POST and return false if date is not set
 		
		if (!isset($_POST['user_name'])) {
 			return FALSE;
 		}
		else {
			$admin_data['user_name'] = $_POST['user_name'];
		}
		...
		if (!isset($_POST['repeat_password'])) {
			...
			$admin_data['repeat_password'] = $_POST['repeat_password'];
		}
		return $admin_data;
 	}
	...
	function validate_unique_user_name($user_name) { //check if user name is not used in the database
		...
		$db_data = mysqli_query($db_link, "SELECT a_id FROM admins WHERE user_name='".$user_name."'"); //search for accounts with the same user name
					...
					<body>
						<div>
							...
							<div>
								...
								<a href="./account.php">Zurück zur Accountverwaltung</a>
							</div>
	...
	function validate_data($admin_data) { //validate all user data (correct length, not null, password and repetition equal), otherwise return false
		
		if(validate_unique_user_name($admin_data['user_name'])) { //check if user name is unique, then validate other data
			
			if (strlen($admin_data['user_name'])>50 || strlen($admin_data['user_name'])==0) {
				return FALSE;
			}
			else if (strlen($admin_data['password'])==0 || $admin_data['password']!=$admin_data['repeat_password']) {
				return FALSE;
			}
			else {
				return TRUE;
	...
	function register_admin_to_database($admin_data) { //register user to database
			
		$admin_data['password']=hash('sha512', $admin_data['password']); //create hash from password
		...
		if (!mysqli_query($db_link, "INSERT INTO admins (user_name, pwhash) VALUES ('".$admin_data['user_name']."', '".$admin_data['password']."')")) {
			return FALSE;
		}
		else {
			return TRUE;
	...
	function register_admin() {	
			$admin_data = get_admin_data(); //get user data
			
			if ($admin_data == FALSE) { //check handle missing data
				die("FEHLER: Fehler beim übermitteln der Daten!");
			}
			else {
				if (!validate_data($admin_data)) { //check and handle incorrect data
					die("FEHLER: Fehlerhafte Daten!");
				}
				else {
					if(!register_admin_to_database($admin_data)) { //register user and handle possible errors
...
//execute registration
	...
	if (register_admin()) { //register user and print success message on success
		?>
			<!DOCTYPE html>
			<html lang="en">
				<head>
