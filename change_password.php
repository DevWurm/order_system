...
function update_password($password_data) { //update password in database
	...
	if (!mysqli_query($db_link, "UPDATE users SET pwhash='".$password_data['pwhash']."' WHERE user_name='".$_SESSION['user_username']."'")) {
		return FALSE;
	}
	else {
		$_SESSION['user_password'] = $password_data['password']; //update current session
		return TRUE;
	}
