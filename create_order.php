...
//local function declarations
	function get_order_data() { //get user input from POST and return false if date is not set
		if(!isset($_POST['product_id'])) {
			return FALSE;
		}
		else {
			$order_data['product_id'] = $_POST['product_id'];
		}
	...
	function validate_data($order_data) { // check if order quantity is not bigger than product quantity
		...
		$db_data = mysqli_query($db_link, "SELECT quantity FROM products WHERE quantity>0 AND p_id='".$order_data['product_id']."'");
		
		mysqli_close($db_link);
		
		if ($db_data != FALSE) { //check if an error occured
			if (mysqli_num_rows($db_data)==1) { //check if product ID exists
				$data = parse_sql_data($db_data); //parse data
			}
			else {
		...
		if ($data[0]['quantity'] < $order_data['product_quantity']) { //compare quantitys
			return FALSE;
		}
		else {
			return TRUE;
	...
	function get_product_price($order_data) { //get price for static_price
				$config = read_config(); //read mysql config
		...
		$db_data = mysqli_query($db_link, "SELECT price FROM products WHERE p_id='".$order_data['product_id']."'");
	...
	function get_user_id() { //get user ID for u_id in orders table
		$config = read_config(); //read mysql config
	...
	function create_order($order_data) { //register order to database
		$order_data['user_id']= get_user_id(); //get user id
		
		$config = read_config(); //read mysql config
		
		$product_price = get_product_price($order_data);
		...
		if (!mysqli_query($db_link, "UPDATE products SET quantity=quantity-'".$order_data['product_quantity']."' WHERE p_id='".$order_data['product_id']."'")) {
			return FALSE;
		}
		else {
			if (!mysqli_query($db_link, "UPDATE users SET number_of_orders=number_of_orders+1 WHERE u_id='".$order_data['user_id']."'")) {
			return FALSE;
			}
			else {
			//send register query to the database and return true at successfull and false at unsuccessfull handeling	
				if (!mysqli_query($db_link, "INSERT INTO orders (u_id, p_id, quantity, order_date, static_price) VALUES ('".$order_data['user_id']."','".$order_data['product_id']."','".$order_data['product_quantity']."',CURDATE(), '".$product_price."')")) {
					return FALSE;
				}
				else {
					return TRUE;
						...
						<body>
							<div>
								...
								<div>							
									...
									<a href="./order.php">Zurück zur Bestellung</a>
								</div>
