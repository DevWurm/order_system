...
//Local function declarations
	...
	function get_user_id() { //get user ID for deletion
	...
	function delete_references($user_id) { //delete referencing table entries
		$config = read_config(); //read mysql config
		...
		if (!mysqli_query($db_link, "DELETE FROM orders WHERE u_id='".$user_id."'")) {
			return FALSE;
		}
		else {
			return TRUE;
	...
	function delete_user($user_id) { //delete user from database
		$config = read_config(); //read mysql config
		...
		//send delete query to the database and return true at successfull and false at unsuccessfull handeling	
		if (!mysqli_query($db_link, "DELETE FROM users WHERE u_id='".$user_id."'")) {
			return FALSE;
		}
		else {
			return TRUE;
