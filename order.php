...
//Local function declarations
	...
	function get_products() { //get all products with quantity bigger than 0
		...
		// send select query
		$db_data = mysqli_query($db_link, "SELECT p_id, product_name, quantity, price FROM products WHERE quantity>0");
			...
			<div>
				...
				if ($products != FALSE) { //if products are existent show products
					...
					for ($i = 0; $i <= count($products)-1; $i++) {
						...
						echo "<input type='number' placeholder='Anzahl (max. ".$products[$i]['quantity'].")' class='product_quantity' name='product_quantity' step='1' min='1' max='".$products[$i]['quantity']."'>";
						echo "<input type='submit' class='product_order' value='Bestellen' />";
						echo "</form><br /><br />";
					}
				}
				else {
					echo "<h2>Keine Produkte auf Lager!</h2>";
				}			
				?>
