...
function get_password_data() { //get user input from POST and return false if date is not set
	if (!isset($_POST['password'])) {
		...
		$password_data['password'] = $_POST['password'];
	...
	if (!isset($_POST['repeat_password'])) {
		...
		$password_data['repeat_password'] = $_POST['repeat_password'];
	}
...
function update_password($password_data) {
	...
	if (!mysqli_query($db_link, "UPDATE providers SET pwhash='".$password_data['pwhash']."' WHERE user_name='".$_SESSION['provider_username']."'")) {
		return FALSE;
	}
	else {
		$_SESSION['provider_password'] = $password_data['password'];
		return TRUE;
	}
